# Analysis of results from simulations where one trait is neutral while all other traits are under stabilizing selection
...
fn=paste("sum_all_",ntrait,"t.txt",sep="")
...
colnames(out)=c("vr","vg","sd_vr","se_vg") # Name columns before writing file
fn=paste("sum_qn_",ntrait,"t.txt",sep="")
...
fn=paste("sum_temp_qn_",ntrait,"t.txt",sep="")
...
h=h+xlab("")+ylab("")
# Save plots
fn1=paste("plot_temp_qn_vr_",ntrait,"t.pdf",sep="")
fn2=paste("plot_temp_qn_vg_",ntrait,"t.pdf",sep="")
...
for(i in 1:length(ntrait.all)){
  ...
  fn=paste("sum_qn_",ntrait.all[i],"t.txt",sep="") # Find the file
...
write.table(d,file="sum_qn.txt",sep="\t") # Write data file

d<-read.table("sum_qn.txt",sep="\t") # (Re-)read data file
...
h=h+xlab("")+ylab("")
# Save plots
ggsave("plot_qn_vg.pdf",plot=g,width=7,height=5)
ggsave("plot_qn_vr.pdf",plot=h,width=7,height=5)
