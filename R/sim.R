...
# Directional selection on two traits
# Selection gradient misaligned with mutational covariance
Nrep=500
T=2e3
opt=c(10,-10) # Optimal phenotype
beta=rbind(c(1,0),c(0,1)) # Matrix of selection
out.all=matrix(0,nrow=Nrep*nrow(par.all)*2,ncol=(T+1+10)) # Matrix that stores all phenotypes through time
no=rep(0,nrow(out.all)) # Vector that stores parameter set index corresponding to each row
colnames(out.all)=c("U1","sig1","U2","sig2","Up","r","siga","sigb","rep","trait",0:T)
...
out.all=data.frame(no,out.all)
write.table(out.all,file="out_dir1.txt",sep="\t")

# Directional selection on two traits + correlational selection
# Selection gradient misaligned with mutational covariance
Nrep=500
T=2e3
opt=c(10,-10) # Optimal phenotype
beta=rbind(c(1,-0.9),c(-0.9,1)) # Matrix of selection
out.all=matrix(0,nrow=Nrep*nrow(par.all)*2,ncol=(T+1+10)) # Matrix that stores all phenotypes through time
no=rep(0,nrow(out.all)) # Vector that stores parameter set index corresponding to each row
colnames(out.all)=c("U1","sig1","U2","sig2","Up","r","siga","sigb","rep","trait",0:T)
for(c in 1:nrow(par.all)){
...
write.table(out.all,file="out_stab1.txt",sep="\t")
write.table(out.end,file="out_stab1_end.txt",sep="\t")

# Stabilizing selection, w/ correlational selection
# PC1 of M matrix and selection gradient misalighed
#Nrep=500
#T=2e4
#opt=c(0,0) # Optimal phenotype
#beta=5*rbind(c(1,-0.5),c(-0.5,1))  # Matrix of selection
#out.all=matrix(0,nrow=Nrep*nrow(par.all)*2,ncol=(T+1+10)) # Matrix that stores all phenotypes through time
#no=rep(0,nrow(out.all)) # Vector that stores parameter set index corresponding to each row
#colnames(out.all)=c("U1","sig1","U2","sig2","Up","r","siga","sigb","rep","trait",0:T)
#for(c in 1:nrow(par.all)){
...
#out.all=data.frame(no,out.all)
#out.end=data.frame(out.all[,1:11],out.all[,ncol(out.all)])
#write.table(out.all,file="out_stab2.txt",sep="\t")
#write.table(out.end,file="out_stab2_end.txt",sep="\t")
