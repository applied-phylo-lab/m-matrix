...
par.default=c(1,0.1,1,0.1,1,0.1,0.1,0.1) # Starting parameter set
...
for(c in 1:nrow(par.all)){
	...
	# Scenario 2: One trait under stabilizing selection, the other is neutral
	...
	# Scenario 3: One trait is under stabilizing selection, the other is under directional selection
	...
	# Scenario 4.1: Both traits are under stabilizing selection (selection aligned w/ mutational covariance)
	...
	# Scenario 4.2: Both traits are under stabilizing selection (selection misaligned w/ mutational covariance)
	sim.out=sim(par,c(2,-2),c(1,1),Ne,Nrep,T)
...
#write.table(data.frame(par.all,out.stabilizing),file="sim_out_stabilizing.txt",sep="\t")
#write.table(data.frame(par.all,out.dir),file="sim_out_directional.txt",sep="\t")
write.table(data.frame(par.all,out.dir.biv2),file="sim_out_directional_biv2.txt",sep="\t")
